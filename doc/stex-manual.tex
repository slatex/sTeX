...
\begin{sfragment}[id=sec.customhighlight]{Highlighting and Presentation Customizations}
  ...
  The \pkg{stexthm} defines some default customizations that can be used, but of course
...
\end{latexcode}
  ...or, rather, since e.g. |theorem|-environments defined using \pkg{amsthm} take an
...
\end{latexcode}
  ...
  Or, if we want all \stexcode"sdefinition"s to use a predefined |definition|-environment,
  we can do
\begin{latexcode}
\stexpatchdefinition
...
\end{latexcode}
  ...
  \begin{function}{\compemph@uri,\varemph@uri,\symrefemph@uri,\defemph@uri}
    ...
    us to e.g. use pdf tooltips and links. For example, this document uses
\begin{latexcode}
