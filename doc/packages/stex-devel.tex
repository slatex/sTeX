...
\begin{sfragment}{Symbols}
  ...
  \begin{itemize}
    ...
    \item its \emph{arity} (a numeric string in the range |0..9|),
    \item a list of its argument specifiers of the form
      \meta{i}\meta{mode}|{|\meta{argname}|}|,
    \item its definiens,
    \item its type,
    \item its return code, and
    \item an invocation macro.
  \end{itemize}
...
\begin{sfragment}{Persisting Content from Math Archives in sms-Files}
  ...
  \begin{sfunction}{stex_persist:n}{\stex_persist:n,\stex_persist:o,\stex_persist:x}
    ...
    \begin{texnote}
      \meta{code} is being process using regular expressions before
      writing out to make it compliant with \pkg{expl3} category
      codes, which is somewhat expensive.
      Care should be taken that \meta{code} is not too big.
    \end{texnote}
  \end{sfunction}

  \begin{sfunction}{stex_persist_in:n,persist:n}{\stex_persist_in:n,\persist:n}
    \begin{syntax}\dcs\marg{code}\end{syntax}
    writes the md5 checksums like \cs{stex_persist:n},
    and subsequently executes \meta{code}. In \meta{code}, we can
    call \dcs[persist:n] to \emph{actually} persist content.
    This wraps all individual code fragments provided in
    \cs{persist:n} with the same file path and md5 checksum,
    and is useful if we want to persist e.g. multiple macro
