...
\begin{sfragment}{Symbols}
  ...
  \begin{sfunction}{_stex_invoke_symbol:nnnnnnnN}{\_stex_invoke_symbol:nnnnnnnN}
    \begin{syntax}\dcs
...
\begin{sfragment}{Imports and Morphisms}
  ...
  \begin{sfunction}{stex_module_add_morphism:nnnn}{\stex_module_add_morphism:nnnn, \stex_module_add_morphism:nonn, \stex_module_add_morphism:oonn}
...
\begin{sfragment}{Structural Features}
  ...
  \stexvarmacro{g_stex_last_feature_str}
  \begin{sfunction}{stex_structural_feature_module:nn,stex_structural_feature_module_end:}{\stex_structural_feature_module:nn,\stex_structural_feature_module_end:}
    ...
    stored in \cs{g_stex_last_feature_str} for 
    subsequent elaboration.
  \end{sfunction}

  \stexvarmacro{l_stex_feature_name_str}
  \stexvarmacro{l_stex_current_domain_str}
  \stexvarmacro{l_stex_morphism_symbols_seq}
  \stexvarmacro{l_stex_morphism_morphisms_seq}
  \begin{sfunction}{stex_structural_feature_morphism:nnn,stex_structural_feature_morphism_end:}{\stex_structural_feature_morphism:nnn,\stex_structural_feature_morphism_end:}
    \begin{syntax}\dcs\marg{name}\marg{typeid}\marg{domain}\end{syntax}
    opens a new morphism-like structural feature of type \meta{typeid} 
    with name \meta{name} and domain \marg{domain}, which needs to be closed with
    ...
    Defines \cs{l_stex_feature_name_str} as \meta{name},
    \cs{l_stex_current_domain_str} as the full uri of \marg{domain},
    \cs{l_stex_morphism_symbols_seq} and
    \cs{l_stex_morphism_morphisms_seq}.
...
\begin{sfragment}{Persisting Content from Math Archives in sms-Files}
  ...
  \begin{sfunction}{stex_persist:n}{\stex_persist:n,\stex_persist:o,\stex_persist:x}
    \begin{syntax}\dcs\marg{code}\end{syntax}
    writes \meta{code} to either the \cs{jobname}|.sms|-file
    or the \cs{jobname}|.deps|-file, depending on whether we are
  ...
  \begin{sfunction}{stex_persist_deps:nn}{\stex_persist_deps:nn,\stex_persist_deps:ox}
    \begin{syntax}\dcs\marg{file}\marg{code}\end{syntax}
    persists \meta{code} explicitly as stemming from 
    the file with string path \marg{file} (regarding the checksum-test) 
    in the |.deps|-file.
  \end{sfunction}
