...
\begin{function}{\STRlabel,\STRcopy}
  ...
  content for later use by |\STRcopy[|\meta{URL}|]{|\meta{label}|}|, which expands to the
  previously stored content. If the |\STRlabel| macro was in a different file, then we can
  give a URL \meta{URL} that lets {\latexml} generate the correct reference.\ednote{MK; rethink}
\end{function}
