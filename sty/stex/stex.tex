...
{\omdoc}~\cite{Kohlhase:OMDoc1.2}. These representation languages for mathematics
...
The promise of these content-oriented approaches is that various tasks involved in ``doing
...
\item reusability, searchability and integration with mathematical software systems
...
documents in {\LaTeX}. It provides \texttt{omdoc.sty}, with the document markup
infrastructure that corresponds to the one in \omdoc and a class 
\texttt{omdoc.sty} that wraps it. 

\subsubsection{Slides and Course Materials}

The \texttt{mikoslides} package provides an infrastructure semantically preloaded slides
and course materials documents in {\LaTeX}. It provides \texttt{mikoslides.sty}, with the
document markup infrastructure that corresponds to the one in \omdoc and a class
\texttt{mikoslides.sty} that wraps it.
